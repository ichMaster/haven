...
A version issues file is the fine-grained breakdown of a ROADMAP version (v0–v3): each phase (`vA.B`) in [specification/ROADMAP.md](../../../specification/ROADMAP.md) is split into one or more `HVN-xxx` issues. If the file does not exist yet, derive it from the version's phases (each phase's Goal / Tasks / DoD) first, then run this skill.
...
- `Title`
- `Size` (S, M, L)
- `Area` (the component: `frontend`, `backend`, `shared`)
- `Phase` (the ROADMAP phase it implements, e.g. `v1.2`)
- `Dependencies` (list of HVN-xxx IDs)

Then parse each **detailed issue section** (heading with HVN-xxx) to extract:
- `Description`
...
```bash
# Version label
gh label create "v1::version:1" --color "0E8A16" --description "Version v1 — FastAPI backend & multiple agents" 2>/dev/null || true
...
gh label create "v1::area:frontend" --color "1D76DB" 2>/dev/null || true
gh label create "v1::area:backend"  --color "6F42C1" 2>/dev/null || true
gh label create "v1::area:shared"   --color "0E8A16" 2>/dev/null || true
```
...
3. Record the mapping: HVN-xxx -> GitHub issue #number

4. Report to user: `Created HVN-xxx -> #{number}: {title}`

5. If the issue has dependencies on already-created issues, add a comment:

```bash
gh issue comment {issue-number} --body "Blocked by #{dep-issue-number} (HVN-xxx)"
```
...
|--------|----------|-------|-------|--------|-----|
| HVN-001 | #5 | FastAPI backend owns the world | v1.1 | v1::version:1, v1::size:M, v1::area:backend | {url} |
| ... | ... | ... | ... | ... | ... |
