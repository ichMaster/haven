...
- `/release-version 1.1.0 FastAPI backend owns the world; GET /world and GET /state over REST` -- bump with provided changelog items
...
3. Summarize the changes into concise bullet points (group related commits; reference the Haven phase `vA.B` where relevant)
...
1. **`README.md`** (if present): update version reference
2. **`RELEASE.txt`** (create if it doesn't exist): prepend a new version block at the top (after the header if any):
...
- **Don't modify source files.** This skill only touches version metadata, not frontend, backend, or shared code.
